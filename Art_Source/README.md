...
| 폴더 | 단계 | 내용 | git |
|------|------|------|-----|
| `prompts/` | ① | 프롬프트 **SoT**. 아래 하위 폴더로 갈려 있다 | ✅ 추적 |
| `prompts/assembled/` | ① | build.py 산출물. **모델에 넣는 건 여기뿐** | ❌ 제외 |
...
> 같은 기준으로 `prompts/assembled/`는 제외한다 — 블록과 `build.py`만 있으면 다시 나온다.

### `prompts/` 하위 구조 (2026-08-20 재편)

```
prompts/
  build.py       조립기
  blocks/        공용 블록 — 참조범위·스타일·정체성·프레이밍·규칙   ❌ 수정 금지
  forms/         폼별 블록 — 몸·무기·실루엣·리깅 배제               ✅ 수정 가능
  poses/         행 이미지용 포즈 문구 예시 (조립 안 됨, 참고용)
  legacy/        블록 구조 이전의 1회성 초안 (안 씀, 이력으로만)
  assembled/     ← build.py 산출물. 모델에 넣는 건 여기뿐
```
...
> 이제 **폴더가 역할을 말한다.** 헷갈릴 자리에서는 `assembled/`만 보면 된다.
...
(정측면·팔다리 분리·관절 가시·천 정지·**빈손**). 규약은 `prompts/blocks/rig_rules.txt`, 근거는 GUIDELINE §7.
...
> 기준 이미지(`base/dark_blade.png`)는 **거대한 대검을 쥐고 있다.** 예전에는 세 용도가
...
⚠️ **`blocks/style.txt`와 `blocks/identity_figure.txt`는 손대지 않는다.** 폼마다 정체성이
