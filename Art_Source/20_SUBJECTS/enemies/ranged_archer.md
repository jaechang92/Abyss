...
| | 무엇 | 언제 |
|---|---|---|
| ⓐ | 🔴 **발사 위치 — 실측 끝, 코드가 남았다.** 지금은 `SpawnProjectile` 이 원점에서 `방향 × (사거리 0.3)` = **(1.5, 0)** 에 낸다(`EnemyBase.Attack.cs:155`). 공격 f4(발사 시각)의 화살촉을 재니 원점 기준 **(+0.9, +0.6)** 유닛이다 → **화살이 0.6유닛 낮게, 배에서 나간다.** `EnemyData` 에 발사 오프셋 필드가 필요하다(기본 0 = 옛 동작, 동작 시간 4개와 같은 철학) | **사용자 승인 대기** |
