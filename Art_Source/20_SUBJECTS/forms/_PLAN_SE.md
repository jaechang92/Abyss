...
폐기: `cb29cabb` Shield Body v1 · `1b0ce390` Thrower Body v1 — 🗑 2026-09-17 계정에서 삭제.
...
| ~~옛 state 정리(삭제)~~ — ✅ 2026-09-17 8개 삭제. 목록은 `_ASSETS.md` 「🗑 삭제」 | R3 이후 |

---

## 🔴 2026-09-17 투척사 몸 교체 — `0f43556f` → `911388e0`

R1' 에서 「발이 안 보여 달리기가 안 읽힐 수 있다」를 감수하고 종 망토(v2)를 채택했는데, 실제로는 그보다 컸다 —
바닥 그림자가 구워졌고, 망토 앞자락 사이에 **다리가 아예 없었고**, Idle 을 네 번 고쳐도 이상했다(사용자 지적 셋).
밑단을 종아리 중간으로 올린 몸으로 교체했다. 키를 맞추려고 **붉은 기사 몸에서 새로** 파생했다(v3 는 다리가 늘어 7px 컸다).

📌 **R1 판정에서 감수한 위험은 R2 에서 반드시 값을 치른다.** 「발이 안 보인다」는 몸 판정 때 이미 보였던 것이다.
상세는 `Art_Source/characters/void_thrower/animation_prompts.md` 맨 위.
