namespace Abyss.Runtime.Camera
{
    ...
    public static class PixelScale
    {
        ...
        /// 제단(`FormAltarBuilder`).
        ///
        /// 🔴 <b>2026-09-10 전수 조사 — 그리고 같은 날 결론을 정정했다.</b>
        /// 처음에 「나머지 PPU 는 다른 축이라 안 묶는다」고 적었는데 <b>전제가 틀렸다.</b>
        /// 폼 128·적 16 은 각자 근거 있는 현재 규약이 아니라 <b>아직 안 갈아엎은 옛 자산의 값</b>이다.
        /// 코드에 남아 있는 설명(`손그림+Bilinear`·`도트`)을 현재 방향으로 읽은 것이 잘못이었다.
        /// <code>
        /// 대상             PPU   아트1px→화면px   상태
        /// 환경·제단         32       3.00        ← 이 상수. 새 파이프라인으로 갈아엎음(stage1 27장)
        /// 캐릭터(새 시트)   32       3.00        2026-09-13 확정. 피벗 BottomCenter. 에디터에서 직접 설정
        /// 폼 스프라이트    128       0.75        옛 자산(2026-08-20 손그림). 새 시트가 없는 폼의 정지 그림 폴백
        /// 적 스프라이트     16       6.00        옛 자산(도트). 픽셀아트로 다시 그릴 것
        /// 흰 사각형(유틸)    4      24.00        4x4 단색. 그림이 아니라 스케일 조절 도구 — 유일하게 무관
        /// UI 아이콘         32        —          Canvas 안이라 월드 배율과 무관. 값이 같은 것은 우연
        /// </code>
        ///
        /// ✅ <b>캐릭터 격자는 환경과 같은 32 로 확정됐다 (2026-09-13).</b> 무기 시트(32)와도 같은 격자이고,
        /// 피벗은 발밑 원점 규약(SoT = `Run` 씬 `Player.prefab`)에 맞춰 BottomCenter 다.
        /// 새 캐릭터 시트(`Art/Sprites/Characters/`)는 임포터 없이 에디터에서 설정했으므로 <b>이 상수를
        /// 참조하지 않는다</b> — 값을 바꿀 때는 그 `.meta` 도 같이 맞춰야 한다.
        /// `FormSpriteImporter`(128)·`PrefabBuilder.Enemies`(16)는 옛 자산용이라 아직 그 값 그대로다.
        /// </summary>
